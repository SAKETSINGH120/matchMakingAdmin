...
import {
  getHeroSection,
  updateHeroSection,
} from "../../Services/HeroSectionApi";
import attachUrl from "../../utils/attachUrl";

const UpdateHeroSectionPage = () => {
  const { id } = useParams();
  ...
  useEffect(() => {
    if (id) {
      const fetchData = async () => {
        setLoading(true);
        try {
          const res = await getHeroSection(id);
          if (res.status) {
            toast.success("Herosection fetched successfully!");
            const { image, type, status } = res.data;
            setFormData({ image: null, type, status });

            // Set image preview correctly with the full URL
            setImagePreview(attachUrl(image));
          }
        } catch (err) {
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      Object.keys(formData).forEach((key) => {
        ...
        } else if (key !== "image") formDataToSend.append(key, formData[key]);
      });

      const res = await updateHeroSection({ id, data: formDataToSend });

      if (res.status) {
        navigate(-1);
        toast.success("HeroSection updated successfully!");
      } else {
        const errorMessage =
      ...
      <div className="ml-5 mt-8 bg-white p-6 max-w-9xl rounded-xl shadow-xl">
        <form onSubmit={handleSubmit}>
          ...
          <input
            ...
            aria-label="Upload HeroSection Image"
          />
          {apiError.image && (
          ...
          >
            {loading ? "Updating..." : "Update HeroSection"}
          </button>
        </form>
      </div>
    </div>
...
export default UpdateHeroSectionPage;
