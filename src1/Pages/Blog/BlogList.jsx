...
import { getAllBlog, BlogDelete } from "../../Services/BlogApi";
...
export default function BlogList() {
  ...
  const fetchData = useCallback(async () => {
    try {
      setLoading(true);
      const result = await getAllBlog({ page, rowsPerPage, searchQuery });
      if (result?.status) {
  ...
  const exportFunc = async (allLeadsData) => {
    if (allLeadsData.length < 1) {
      return toast.error("Blog list is empty!");
    }
    setIsExporting(true);
    ...
    const data = [
      {
        ...
        columns: [
          ...
          { label: "Title", value: (row) => row?.title || "" },
          {
            label: "Thumb Image",
            value: (row) =>
              row?.thumbImage ? joinUrl(BASE_URL, row.thumbImage) : "",
          },
          {
            label: "Images",
            value: (row) =>
              Array.isArray(row?.image)
                ? row.image.map((img) => joinUrl(BASE_URL, img)).join(", ")
                : "",
          },
  ...
  return (
    <div className="p-6 bg-gray-50 min-h-screen">
      ...
      <div className="flex justify-between items-center mb-8">
        ...
        <div className="flex gap-4">
          <motion.button
            ...
            aria-label="Export blogs to Excel"
          >
            {isExporting ? (
      ...
      >
        <Table sx={{ minWidth: 700 }} aria-label="blog table">
          <TableHead>
            <TableRow>
              <StyledTableCell>S.No</StyledTableCell>
              <StyledTableCell>Thumb Image</StyledTableCell>
      ...
      {totalRecord > rowsPerPage && (
        <Stack spacing={2} alignItems="center" marginTop={6}>
          <Pagination
            ...
            boundaryCount={1}
            siblingCount={1}
            aria-label="Pagination for blog list"
          />
        </Stack>
      )}
    </div>
