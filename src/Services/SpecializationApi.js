...
export const getAllBanner = async ({ page, rowsPerPage, searchQuery }) => {
  const token = localStorage.getItem('token');
    try {
      ...
      const res = await fetch(`${BASE_URL}/api/admin/specialization?search=${searchQuery}&page=${page}&limit=${rowsPerPage}`, {
...
//   };
   ...
   export const createStreamApi = async (data) => {
    ...
    console.log(data, ' data for send in the api');
    try {
      const res = await fetch(`${BASE_URL}/api/admin/specialization`, {
        method: 'POST',
        headers: {
          Authorization: `Bearer ${token}`,
  ...
  const token = localStorage.getItem('token');
  try {
    const res = await fetch(`${BASE_URL}/api/admin/specialization/${id}`, {
      method: 'DELETE',
      headers: {
        Authorization: `Bearer ${token}`,
        'Content-Type': 'application/json'
