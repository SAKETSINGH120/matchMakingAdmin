...
import { createHeroSection } from "../../Services/HeroSectionApi";

const AddNewHeroSectionPage = () => {
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      const res = await createHeroSection(formDataToSend);

      if (res.status) {
        navigate(-1);
        toast.success("HeroSection added successfully!");
      } else {
        const errorMessage =
    ...
    <div className="m-3">
      ...
      <div className="ml-5 mt-8 bg-white p-6 max-w-9xl rounded-xl shadow-xl">
        <form onSubmit={handleSubmit}>
          ...
          >
            {loading ? "Creating..." : "Create HeroSection"}
          </button>
        </form>
      </div>
    </div>
...
export default AddNewHeroSectionPage;
