...
import { getAllBanner, BannerDelete } from "../../Services/SpecializationApi";
...
const StyledTableCell = styled(TableCell)(({ theme }) => ({
  [`&.${tableCellClasses.head}`]: {
    background: "linear-gradient(to right, #181e2a, #232a3b)",
...
export default function BannerList() {
  ...
  const fetchData = useCallback(async () => {
    try {
      ...
      if (result) {
        toast.success("Specialization fetched successfully!");
        ...
        toast.error(result?.message || "Failed to fetch Specialization.");
      }
    } catch (error) {
      console.error("Error fetching Specialization:", error);
      toast.error("Error fetching Specialization.");
    } finally {
      setLoading(false);
    }
      ...
      onOk: async () => {
        try {
          ...
          if (result?.status) {
            ...
            toast.error(result?.message || "Failed to delete Specialization.");
          }
        } catch (error) {
  ...
  const exportFunc = async (allLeadsData) => {
    if (allLeadsData.length < 1) {
      return toast.error("Specialization list is empty!");
    }
    setIsExporting(true);
    const settings = {
  ...
  // if (authLoading.profile || !auth.user) return <Loader />;
    ...
    <div className="p-6 bg-gray-50 min-h-screen">
      ...
      <div className="flex justify-between items-center mb-8">
        ...
        <div className="flex gap-4">
          ...
          {hasPermission("Specialization", "create") && (
            <motion.button
              whileTap={{ scale: 0.95 }}
      ...
      >
        <Table sx={{ minWidth: 700 }} aria-label="Specialization table">
          <TableHead>
            <TableRow>
              ...
              data.map((row, index) => (
                <StyledTableRow key={row.id}>
                  ...
                  <StyledTableCell align="center">
                    {(hasPermission("Specialization", "read") ||
                      hasPermission("Specialization", "update") ||
                      hasPermission("Specialization", "delete")) && (
                      <IconButton
                    ...
                    >
                      {/* {hasPermission("Specialization", "read") && (
                        <MenuItem
                          onClick={() => {
                            navigate(`SpecializationView/${row.id}`);
                            handleMenuClose();
                          }}
                      ...
                      {hasPermission("Specialization", "update") && (
                        <MenuItem
                          onClick={() => {
                            navigate(`updateSpecialization/${row.id}`);
                            handleMenuClose();
                          }}
                          className="flex items-center gap-2 text-gray-700 hover:bg-gray-100"
                        >
      ...
      {totalRecord > rowsPerPage && (
        <Stack spacing={2} alignItems="center" marginTop={6}>
          <Pagination
            ...
            boundaryCount={1}
            siblingCount={1}
            aria-label="Pagination for Specialization list"
          />
        </Stack>
      )}
    </div>
